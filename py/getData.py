...
filepath = 'E:\\BlendData\\'#定义数据文件保存路径
#实施抓取
...
#数据库名称和密码
name = 'root'
password = 'usbw'  #替换为自己的账户名和密码
#建立本地数据库连接(需要先开启数据库服务)
db = pymysql.connect('localhost', name, password, charset='utf8')
cursor = db.cursor()
#创建数据库blend
sqlSentence1 = "create database blend"
cursor.execute(sqlSentence1)
sqlSentence2 = "use blend;"
cursor.execute(sqlSentence2) # 选择使用当前数据库
...
#重新建立数据库连接
db = pymysql.connect('localhost', name, password, 'blend')
cursor = db.cursor()
#查询数据库并打印内容
